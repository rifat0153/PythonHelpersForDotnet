...
class DapperRequestGenerator:
    ...
    def generate(sp_name : str, params_dict: Dict[str, str]) -> str:
        ...
        request_name_striped = sp_name.replace("usp_", "").replace(".sql", "").strip().rstrip().lstrip()
        ...
        for param_key, param_value in params_dict.items():
            if param_value["direction"] != "OUT":
                request_params.append(f"public {param_value['csharp_type']} {param_value['camel_case_name']} {{ get; init; }}")
